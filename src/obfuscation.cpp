...
/* *** BEGIN OBFUSCATION MAGIC - EMPIRECOIN **********
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat EmpireCoin and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 EMPIRECOIN per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeEmpireCoinAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100EMPIRECOIN+1 ( bit on if present )
    // bit 1 - 10EMPIRECOIN+1
    // bit 2 - 1EMPIRECOIN+1
    // bit 3 - .1EMPIRECOIN+1
    // bit 3 - non-denom
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("empirecoin-obfuscation");
