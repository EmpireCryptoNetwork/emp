...
#include "zempirecoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zEMP ought to be enough for anybody." - Bill Gates, 2017
    ui->zEMPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzEMPSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzEMP->setEnabled(false);
        ui->pushButtonMintzEMP->setToolTip(tr("zEMP is currently disabled due to maintenance."));

        ui->pushButtonSpendzEMP->setEnabled(false);
        ui->pushButtonSpendzEMP->setToolTip(tr("zEMP is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zEMPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzEMP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEMP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zEMP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zEMP in ") +
...
void PrivacyDialog::on_pushButtonSpendzEMP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zEMP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zEMP
        sendzEMP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zEMP
    sendzEMP();
}

void PrivacyDialog::on_pushButtonZEmpireCoinControl_clicked()
{
    ZEmpireCoinControlDialog* zEMPControl = new ZEmpireCoinControlDialog(this);
    zEMPControl->setModel(walletModel);
    zEMPControl->exec();
}

void PrivacyDialog::setZEmpireCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzEMPSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzEMP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid EmpireCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zEMPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zEMP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " EMPIRECOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zEMP</b>";
    ...
    // use mints from zEMP selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZEmpireCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZEmpireCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zempirecoin selector in case it was used
    ZEmpireCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zEMP Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " EmpireCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zEMP Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zEMP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zEMP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zEMP "));
    ui->labelzEMPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzEMPSyncStatus->setVisible(fShow);
}
