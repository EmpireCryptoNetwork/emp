...
#include "ui_zempirecoincontroldialog.h"
...
std::list<std::string> ZEmpireCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZEmpireCoinControlDialog::listMints;

ZEmpireCoinControlDialog::ZEmpireCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZEmpireCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZEmpireCoinControlDialog::~ZEmpireCoinControlDialog()
{
    delete ui;
}

void ZEmpireCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEmpireCoinControlDialog::updateList()
{
...
void ZEmpireCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEmpireCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZEmpireCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEmpireCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZEmpireCoinControlDialog::GetSelectedMints()
{
...
void ZEmpireCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
